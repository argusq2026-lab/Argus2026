...
class ScoringConfig:
    ...
    def weights_for(self, exercise: str) -> dict[str, float]:
        ...
        means the phone reported no exercise at all, which is the same case.
        """
        if not exercise:
            return self.weights
        return self.exercise_weights.get(exercise.lower(), self.weights)

    def profile_name_for(self, exercise: str) -> str:
        """Which profile `exercise` actually resolves to, or `""` for default.

        The console needs this to say *why* a trainee is being scored the way
        they are. A plank scored on the plank profile shows no fall and no
        stillness reason, which is correct and completely opaque unless the
        page can also say that those features are switched off for planks.
        """
        if exercise and exercise.lower() in self.exercise_weights:
