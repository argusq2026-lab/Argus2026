...
Argus watches a floor of people through one phone per person and produces a
**deterministic, explainable triage rank** of who needs a human right now.

Each phone runs its own on-device perception — pose, plus whatever classifier
its domain needs — and streams the numeric result to a laptop over WebSocket;
no video ever leaves the phone. The laptop ranks every connected station with a
pure, auditable scorer and gives the supervisor a live view.
...
**What the floor is doing is a parameter, not an assumption.** A `use_case`
chosen at handshake selects the message shape, the scorer, and the console's
drawing — so a gym, a nursing skills lab, and a welding bay are three
configurations of one engine rather than three forks of it. Fitness is the use
case built out far enough to demonstrate the whole idea end to end; nursing/CPR
is a second, independently-scored one; welding is a registered placeholder that
proves a use case with *no* classifier still connects, streams, and appears on
the console. See [Use cases](#use-cases) and
...
repository — a dashboard front door opens the station screen for the use case
you pick (Fitness and Nursing are built; Lab and Welding are named
placeholders), runs detection and pose on the phone's own NPU, and streams the
result to this server. See [`android/README.md`](android/README.md) for what it
runs and [`docs/PROTOCOL.md`](docs/PROTOCOL.md) for the exact wire contract it
implements. [`demo/replay_client.py`](demo/replay_client.py) is a reference
client that stands in for a real phone when developing the laptop side without
one — an iOS app does not exist and is not started.

---

## Use cases

One `[session] use_case` per floor, declared by every phone at `hello` and
rejected on mismatch. What each one is today:

| Use case | Wire payload | Scored by | State |
|---|---|---|---|
| **`fitness`** | COCO-17 pose + `exercise`, `rep_count`, `form_ok`, closed-vocabulary `form_reason_codes` | Five weighted features on the laptop, with per-exercise weight profiles; form judged **on the phone** | **The worked example.** Three on-device form classifiers (plank, bicep, lunge), two geometric fault checks, rep counting |
| **`nursing`** | COCO-17 pose + `procedure` | The laptop, per procedure. `cpr` measures compression rate, cadence regularity, and elbow lock from the pose itself | **Real, one procedure.** No phone-side classifier at all — the fault is derived from the movement |
