...
| `exercise` | string | no | The exercise being performed (e.g. `"plank"`, `"squat"`). **Selects the server's scoring weight profile** — see "The exercise field is load-bearing" below — and is also shown on the trainer console. Lowercased on the server; unlike `form_reason_codes` it is *not* a closed vocabulary, and an exercise the server has no profile for scores on the default weights rather than being rejected. At most 64 characters: it is the one field on the wire whose value a phone chooses freely and it reaches a human's screen, so a longer value is a malformed message rather than a plausible label. |
| `rep_count` | int | no | Running rep count for the current set. **Display-only** — shown on the console, never scored. Must be a non-negative integer; `true`/`false` is rejected rather than counted as 1. |
...
All three are validated and carried through to the trainer console (see
[`CONSOLE.md`](CONSOLE.md)), but they are not equally inert. `exercise`
chooses *which* weights run; `rep_count` and `form_ok` are read by nothing at
all. That line is deliberate: a weight profile is a config lookup an operator
can inspect, whereas scoring a phone-maintained counter or the phone's own
verdict would make the rank a function of an unauditable device-side value —
`form_reason_codes` carries the same information through a closed vocabulary
that can be checked.
